...
estimator= EstimatorClassEkfExpLinErr(imu.msmt(1:3, 1:params.num_epochs_static), params);
im= IntegrityMonitoringClassEkfExpLinErr(params, estimator);
...
estimator.linearize_discretize( imu.msmt(:,1), params.dt_imu, params, 0);
...
for epoch= 1:imu.num_readings - 1
    ...
    if (counters.time_sim + params.dt_imu) > counters.time_gps
        ...
        if ~params.SWITCH_CALIBRATION && params.SWITCH_GPS_UPDATE
            ...
            estimator.linearize_discretize( imu.msmt(:,epoch+1), params.dt_imu, params, epoch); %Osama
