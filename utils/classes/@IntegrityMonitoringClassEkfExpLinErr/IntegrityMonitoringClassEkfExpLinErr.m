...
classdef IntegrityMonitoringClassEkfExpLinErr < handle
    properties (Constant)
        m= 3
    ...
    methods
        % ----------------------------------------------
        % ----------------------------------------------
        function obj= IntegrityMonitoringClassEkfExpLinErr(params, estimator)
            ...
            obj.q_ph_ub=  ones(params.M, 1) * (-1);
            obj.L_ph=     cell(1, params.M);
            obj.L_ph_ub=  cell(1, params.M);
            obj.Lpp_ph=   cell(1, params.M + 1); % need an extra epoch here (osama)
            obj.Lpp_ph_ub=cell(1, params.M + 1);
            obj.Y_ph=     cell(1, params.M);
            obj.Y_ph_ub = cell(1, params.M);
