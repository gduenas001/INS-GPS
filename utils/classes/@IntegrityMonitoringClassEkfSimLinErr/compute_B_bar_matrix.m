...
obj.B_bar_ub= inf* ones( obj.n_M , obj.n_M + obj.m );
A_prev_ub= obj.Lpp_k_ub \ obj.A_M_ub( : , estimator.n_k + 1:end );
...
if estimator.n_k > 0
    ...
    obj.B_bar_ub(1:estimator.n_k , :)=...
    [ eye(estimator.n_k), -obj.H_k * obj.Phi_ph{1} * A_prev_ub ];
end

% Recursive computation of B
for i= 1:obj.M
    A_prev= obj.Lpp_ph{i} \ A_prev(:, obj.n_ph(i)+1:end);
    A_prev_ub= obj.Lpp_ph_ub{i} \ A_prev_ub(:, obj.n_ph(i)+1:end);
    ...
    if obj.n_ph(i) > 0
        ...
        B_ub= [eye( obj.n_ph(i) ) , -obj.H_ph{i} * obj.Phi_ph{i+1} * A_prev_ub];
        ...
        obj.B_bar_ub(B_ind_row_start:B_ind_row_end, 1:B_ind_col_end)= 0;
        obj.B_bar_ub(B_ind_row_start:B_ind_row_end, B_ind_col_end+1:end)= B_ub;
