...
if  ( params.SWITCH_FIXED_LM_SIZE_PH ...
    ...
    obj.n_total= obj.n_M + obj.n_M_gps + (obj.M + 1) * (9);
    ...
    estimator.PX= obj.PX_M( end - 9 + 1 : end, end - 9 + 1 : end );
    
    % find the prior covarince matrix for time k+1
    obj.PX_prior= obj.PX_M( 9 + 1 : 2*9, 9 + 1 : 2*9 );
    ...
    for i= 0:obj.n_H   
        ...
        if i == 0
            ...
            if obj.n_M+obj.n_M_gps < 9 + sum(obj.inds_H{i}<=obj.n_L_M)*params.m_F + sum(obj.inds_H{i}>obj.n_L_M)*6
