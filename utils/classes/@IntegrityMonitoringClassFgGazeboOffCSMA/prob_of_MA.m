...
for t= 1:estimator.n_L_k
    ...
    if isempty(obj.A)
        obj.mu_k = 0;
%    elseif obj.n_M < 9 + obj.n_max*9_F
%        obj.kappa= 1;
%        obj.G_l_t= obj.kappa * ( sqrt(obj.T_d) - sqrt( chi2inv(1 - params.I_MA , obj.n_M) ) )^2;
        ...
        H_t(:,end-9+1)= [-cpsi;spsi];
        H_t(:,end-9+2)= [-spsi;-cpsi];
        H_t(:,end-9+params.ind_yaw)= [-dx*spsi + dy*cpsi;-dx*cpsi - dy*spsi ];
        ...
        for j= 1:(obj.n_max)%(obj.n_L_M-3)
            C = nchoosek(1: (obj.n_L_M+(obj.n_M_gps/6)) ,j); %set of possible fault indices for n_max simultanous faults
            for i= 1:size(C,1)
        ...
        obj.G_l_t= obj.kappa * ( sqrt(obj.T_d) - sqrt( chi2inv(1 - params.I_MA , obj.n_M + obj.n_M_gps) ) )^2;
    end
