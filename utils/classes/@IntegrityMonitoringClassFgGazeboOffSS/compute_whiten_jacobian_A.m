...
% plug the prior in A
obj.A( 1:9, 1:9 )= sqrtm( obj.Gamma_prior ); %Osama params.m

% pointers to the next part of A to be filled
r_ind= 9 + 1; %Osama params.m
c_ind= 1;
...
for i= obj.M-1 : -1 : 0
    ...
    if i == 0
        ...
        obj.A( r_ind : r_ind + 9 - 1, c_ind : c_ind + 9 - 1)= ...
            sqrtm( inv(estimator.D_bar) ) * estimator.Phi_k; %Osama params.m
        
        obj.A( r_ind : r_ind + 9 - 1, c_ind + 9 : c_ind + 2*( 9) - 1)= ...  %Osama params.m
            -sqrtm( inv(estimator.D_bar) );
        
        
        % update the row & column indices
        r_ind= r_ind + 9;  %Osama params.m
        c_ind= c_ind + 9;  %Osama params.m
        ...
        if estimator.n_gps_k ~= 0
            ...
            obj.A(  r_ind : r_ind + estimator.n_gps_k - 1,...
                c_ind : c_ind + 9 - 1 )= ...  %Osama params.m
                estimator.H_k_gps;
        ...
        % Whiten IMU model and then plug it in A
        obj.A( r_ind : r_ind + 9 - 1, c_ind : c_ind + 9 - 1)= ...  %Osama params.m
